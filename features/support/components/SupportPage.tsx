...
function NewTicketModal({
  ...
  const CATEGORY_LABEL: Record<TicketCategory, string> = {
    technical: t.support.catTech,
    account:   t.support.catAccount,
    club:      t.support.catClub,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" dir={dir}>
      <div className="bg-white rounded-[28px] shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="relative bg-[#21166A] px-6 py-5 flex items-center justify-between">
          <h2 className="text-base font-black text-white">{t.support.newTicketTitle}</h2>
        ...
        <div className="p-6 space-y-4">
          <div>
            <p className="text-xs font-bold text-gray-400 mb-2">{t.support.category}</p>
          ...
          <div>
            <p className="text-xs font-bold text-gray-400 mb-2">{t.support.subject}</p>
            <input
              value={subject}
          ...
          <div>
            <p className="text-xs font-bold text-gray-400 mb-2">{t.support.details}</p>
            <textarea
              value={message}
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={onClose} className="flex-1 py-3 rounded-2xl border border-gray-100 text-sm font-bold text-gray-500 hover:bg-gray-50 transition">{t.cancel}</button>
            <button onClick={handleSubmit} disabled={loading}
              ...
              {loading ? <><Loader2 size={14} className="animate-spin" />{t.sending}</> : t.send}
            </button>
          </div>
        </div>
      </div>
...
function TicketRow({
  ...
  const STATUS_CONFIG: Record<TicketStatus, { label: string; style: string }> = {
    open:        { label: t.support.open,       style: "bg-blue-50 text-blue-600" },
    in_progress: { label: t.support.inProgress, style: "bg-amber-50 text-amber-600" },
    resolved:    { label: t.support.resolved,   style: "bg-emerald-50 text-emerald-600" },
  };
        ...
        <div className="border-t border-gray-50 px-5 py-4 space-y-4">
          {isAdmin && (
            <div className="bg-[#F7F5FF] rounded-2xl px-4 py-3">
              <p className="text-[10px] font-bold text-gray-400 mb-1">{t.support.from} {ticket.userName} · {ticket.userEmail}</p>
              <p className="text-[10px] font-bold text-gray-400">{ticket.universityName}</p>
            </div>
          )}

          <div>
            <p className="text-xs font-bold text-gray-400 mb-1.5">{t.support.message}</p>
            <p className="text-sm text-gray-600 leading-6 bg-gray-50 rounded-2xl px-4 py-3">{ticket.message}</p>
          </div>

          {!isAdmin && ticket.adminReply && (
            <div>
              <p className="text-xs font-bold text-[#7C3AED] mb-1.5">{t.support.adminReply}</p>
          ...
          {isAdmin && (
            <div className="space-y-3">
              <div>
                <p className="text-xs font-bold text-gray-400 mb-1.5">{t.support.reply}</p>
                <textarea
                  value={reply}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-[#7C3AED] mb-0.5">
                  {isAdmin ? t.support.adminTag : t.support.tag}
                </p>
        ...
        <div className="flex-1 p-6 lg:p-8" dir={dir}>
          <div className="max-w-2xl space-y-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: t.support.open,       val: open,       icon: <Clock size={14} className="text-blue-500" />,    style: "bg-blue-50" },
                { label: t.support.inProgress, val: inProgress, icon: <AlertCircle size={14} className="text-amber-500" />, style: "bg-amber-50" },
                { label: t.support.resolved,   val: resolved,   icon: <CheckCircle size={14} className="text-emerald-500" />, style: "bg-emerald-50" },
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-[24px] p-10 shadow-md text-center">
                ...
                <p className="text-sm font-bold text-gray-400">
                  {isAdmin ? t.support.noTicketsAdmin : t.support.noTickets}
                </p>
                {!isAdmin && (
